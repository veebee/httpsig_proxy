...
const certs = require('../config/certificates.json');

//Create HTTP server, listening on port defined in configuration
...
function onRequest(req,res)
{
  ...
  req.on('end',() => {
    ...
    console.log('QWAC key:' + certs.qwac_key);
    console.log('Remove Digest Flag:' + certs.remove_digest);
    console.log('============================================');
    ...
    var options = {
      ...
      method: req.method,
      headers: headers,
      key: fs.readFileSync(certs.qwac_key),
      cert: fs.readFileSync(certs.qwac_cert),
    };
...
function generateBGSignature(headers, body,method) {
    ...
    let QSEALCert=fs.readFileSync(certs.qseal_cert,'ascii');
    let QSEALKey=fs.readFileSync(certs.qseal_key,'ascii');
    ...
    let signingString="";
    if(method!="GET" || "TRUE" != certs.remove_digest)
    {
